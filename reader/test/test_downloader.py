...
def downloader(mocker, test_manga):
    mocker.patch('reader.sources.mangareader.MangaReader.get_manga', return_value=test_manga)
    yield Downloader(source=MangaReader, manga_home=TEST_DATA_DIR)
...
def test_download_manga(mocker, requests_mock, downloader, test_manga):
    mocker.patch('reader.sources.MangaReader.get_page_url', return_value=IMAGE_URL)
    ...
    page = test_manga.chapters[chapter][-1]  # get last page for assert_called_with
...
def test_download(downloader, test_manga):
    chapter = list(test_manga.chapters.keys())[0]
    page = test_manga.chapters[chapter][0]
    downloader.download_manga(test_manga.title)
    chapter_dir = os.path.join(TEST_DATA_DIR, test_manga.title, str(chapter))
