...
from mangareader.utils import Downloader
from mangareader.manga import Manga, Chapter, Page

TEST_DATA_DIR = os.path.join(pytest.TEST_DIR_BASEPATH, 'test_downloads')
...
@pytest.fixture
def downloader(mocker, clean_test_subtree):
    yield Downloader(manga_home=TEST_DATA_DIR)
    clean_test_subtree(TEST_DATA_DIR)


def test_download_manga(mocker, requests_mock, downloader, test_manga):
    mocker.patch('mangareader.sources.mangareader.MangaReaderScraper._get_page_url', return_value=IMAGE_URL)
...
def test_download_chapter(mocker, downloader, test_manga):
    mocker.patch('mangareader.utils.downloader.Downloader._ChapterDownloader._download_page')
...
def test_skip_download_if_chapter_exists(mocker, downloader, test_manga):
    ...
    mocker.patch('mangareader.utils.downloader.Downloader._ChapterDownloader._download_page', side_effect=Exception)
    downloader.download_manga(test_manga)
